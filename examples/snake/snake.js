...
(function() {
  (function() {
    var aghs, board, game, gameover, keyboard, powerup, rand3Ints, reset, secret, snake, ticks, winner, world;
    if (!window.Aghs) {
    ...
    board = {
      ...
      render: function(time, $) {
        $.fillStyle(this.color).fillRect(this.x, this.y, this.width, this.height);
        if (window.location.search.length > 0 && snake.length >= 4) {
          return secret($);
        }
      }
    };
    ...
    secret = function($) {
      ...
      x = (board.width - metrics.width) / 2;
      y = (board.height + size) / 2;
      return $.fillStyle(c).fillText("NUDES", x, y);
    };
    aghs.state(game);
    aghs.state(gameover);
    return aghs.start();
